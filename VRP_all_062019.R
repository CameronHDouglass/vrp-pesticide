...
rownames(summ_inputs)<-c("adLD50","adslope","lLD50","lslope",
...
for(i in 1:nsims){
  ...
  cadnectar <- runif(1, 133, 337) 
  cadpollen <- runif(1, 0, 1) 
  cl4nectar <- runif(1, 0, 100) 
  cl4pollen <- runif(1, 0, 4)
  cl5nectar <- runif(1, 0, 150) 
  cl5pollen <- runif(1, 0, 6) 
  cldnectar <- runif(1, 0, 160) 
  cldpollen <- runif(1, 0, 5) 
  frgrnectar <- runif(1, 80.25, 321) #CForagerNectar
  frgrpollen <- runif(1, 8.75, 13) #CForagerPollen
  nload <- 30 #runif(1, 22, 51) #INectarLoad
  ntrips <- 17 #rlnormTrunc(1, meanlog=10, sdlog=3, min=1, max=17) #INectarTrips
  pload <- runif(1, 15, 35) #IPollenLoad
  ptrips <- rlnormTrunc(1, meanlog=10, sdlog=3, min=1, max=24) #IPollenTrips
  initialnectar <- rtruncnorm(1, a=11408, b=19571, mean=15490, sd=7101) #InitColNectar
  initialpollen <- rtruncnorm(1, a=1775, b=2921, mean=2348, sd=997) #InitColPollen
...
necpolfile <- npcontamination_level3
